...
\\
\\
\tab We will use two different constraint specifications for the pizza problem and will compare them. While writing the specification on \textbf{Minizinc}, there was an impression that when we specify constraints in linear form, where "linear" means that the problem is not specified nestedly such as \textbf{constraint forall}($p$ \textbf{in} $1..n$, $c$ \textbf{in} $1..m$)($...$), they seem to be optimized and perform faster than their nested equivalent form \textbf{constraint forall}($p$ \textbf{in} $1..n$)(\textbf{forall} $c$ \textbf{in} $1..m$)($...$)). We will then perform our test instances in each one of the two slightly different specifications, measure and compare their running time.
\\
\\
